...
- `id` は ASCII の識別子 (`[A-Za-z0-9_]+`) のみ。数字のみのID (例: `6`) も可。日本語等は不可
- ジャンプ時点で外側の branch スタックは全て破棄されます。入れ子の内側 branch から `-> id` した場合も同じく外側へは戻りません

### branch にポイントを加算する / 表示条件をつける

branch ヘッダは `- ボタン文字列 (効果) [表示条件] -> next` の形で、**効果** と **表示条件** を任意でつけられます。書く順番は「効果 → 表示条件 → next」。

```markdown
? losing_things

- 「別にいいじゃないですか」 (aobaPoints+1)
    瑞穂: 落とすときは落とすでしょう
...
- 保管部へ [aobaPoints>=3] -> end-hokanbu
    ...
- 弓道部へ -> end-kyuudou
    ...
- 一人で [aobaPoints<3] -> end-maverick
    ...
```

- **効果** `(変数名+N, 変数名-N, ...)`
  - その branch を選んだとき、`GameState.points[変数名]` に加算する (初期値 0)。カンマ区切りで複数指定可
  - 変数名は ASCII (`[A-Za-z_]\w*`)。値は整数。例: `(aobaPoints+1)`, `(aobaPoints+2, trust-1)`
- **表示条件** `[変数名 OP N, ...]`
  - 全条件を満たすときだけ選択肢を表示する (満たさなければウィンドウに出ない)。カンマ区切りは AND
  - OP は `>=` `<=` `>` `<` `==` `!=`。比較対象の変数が未設定なら 0 として扱う。例: `[aobaPoints>=3]`
  - 条件で絞り込んでも内部のインデックスは engine 側で吸収されるので、選んだ branch は正しく実行される
- ボタン文字列に効果・条件の区切り文字を使いたいときは、ASCII の `()` `[]` ではなく**全角** `（）` `［］` を使うこと (ASCII だと効果/条件として誤解析される)
...
| `選択肢の効果が不正`                | `( )` 内が `変数名+N` の形でない            |
| `選択肢の表示条件が不正`            | `[ ]` 内が `変数名 OP N` の形でない         |
